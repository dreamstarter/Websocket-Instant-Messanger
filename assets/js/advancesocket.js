...
var AdvancedSocket = {
  ...
  channels: document.body.dataset.channels
    ? document.body.dataset.channels.split(",")
    : [],
  clientID: 0,
  clientInfo: {},
  ...
  reconnectTimer:
    (parseFloat(document.body.dataset.reconnectTimer) || 0.5) * 1000,
  timerCount: 0,
  debug: JSON.parse(document.body.dataset.debug || false),
  statusLabel: document.getElementById(
    document.body.dataset.statusDiv || "status-message"
  ),
  ...
  checkConnection: function() {
    ...
    if (
      navigator.onLine &&
      AdvancedSocket.pingURL !== "" &&
      AdvancedSocket.autoConnect
    ) {
  ...
  ping: function(url) {
    ...
    xhr.open(
      "GET",
      url + "?id=" + AdvancedSocket.clientID + "&ts=" + new Date().getTime()
    );
    xhr.send();
  },

  onMessage: function(obj) {
    AdvancedSocket.doLog(
      "AdvancedSocket : onMessage",
      obj.type,
      obj.reqType,
      obj
    );
    ...
    if (obj.type === "data") {
      ...
      if (obj.data === "FORCE-RECONNECT") {
        window.setTimeout(
          AdvancedSocket.forceReconnect,
          AdvancedSocket.reconnectTimer
        );
      }

      // if we defined a global doMessage function
      if (
        AdvancedSocket.doMessage &&
        typeof window[AdvancedSocket.doMessage] === "function"
      ) {
      ...
      // notify user to create required notification
        ...
        AdvancedSocket.doLog(
          "AdvancedSocket : Create a doMessage function and pass it in the data-do-message attribute of the body"
        );
      }
    }
  },
  ...
  onOpen: function(obj) {
    ...
    if (
      window.AdvancedSocket.clientInfo.username &&
      AdvancedSocket.autoConnect
    ) {
      AdvancedSocket.autoConnect = false;
      window[AdvancedSocket.name].authenticate(
        window.AdvancedSocket.clientInfo.username,
        ""
      );
    }
  ...
  getIPInfo: function() {
    ...
    if (
      window.jQuery &&
      !AdvancedSocket.clientInfo.status &&
  ...
  connectWS: function() {
    ...
    AdvancedSocket.channels.forEach(function(value, index) {
      AdvancedSocket.doLog(
        "AdvancedSocket : Connecting to " +
          value +
          " : " +
          (index + 1) +
          " of " +
          AdvancedSocket.channels.length
      );
...
AdvancedSocket.doLog =
  AdvancedSocket.debug === true && window.console
